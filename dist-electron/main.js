...
function checkNmapInstallation() {
    ...
    return new Promise((resolve) => {
        try {
            if (process.platform === "win32") {
                ...
                try {
                    (0, child_process_1.execSync)("where nmap");
                    nmapInstallationChecked = true;
                    isNmapAvailable = true;
                    resolve(true);
                }
                ...
                try {
                    (0, child_process_1.execSync)("which nmap");
                    nmapInstallationChecked = true;
                    isNmapAvailable = true;
                    resolve(true);
                }
...
electron_1.app.whenReady().then(() => {
    ...
    (0, plugin_manager_1.setupPluginIpcHandlers)();
    // Load initial scripts
    const pluginManager = plugin_manager_1.PluginManager.getInstance();
...
function getNmapPath() {
    if (process.platform === "win32") {
        ...
        try {
            return (0, child_process_1.execSync)("where nmap").toString().trim();
        }
        catch {
        ...
        try {
            return (0, child_process_1.execSync)("which nmap").toString().trim();
        }
        catch {
            return "nmap"; // fallback
        }
...
// Map to keep track of active nmap processes
const activeNmapProcesses = new Map();
// Handle Nmap scan requests
electron_1.ipcMain.handle("run-nmap-scan", async (_, target, parameters) => {
    return new Promise((resolve) => {
        checkNmapInstallation().then((nmapInstalled) => {
            if (!nmapInstalled) {
                ...
                electron_1.dialog.showErrorBox("Nmap Not Found", errorMessage);
                resolve({
                    success: false,
                    error: errorMessage,
                });
            ...
            const processedParams = parameters.map((param) => {
                ...
                if (param.startsWith("--script=") && !param.includes("/")) {
                    ...
                    const scriptPaths = scriptNames.map((name) => scriptMap.has(name) ? scriptMap.get(name) : name);
                    return `--script=${scriptPaths.join(",")}`;
                }
                return param;
            });
            const args = [...processedParams, target];
            const nmap = (0, child_process_1.spawn)(nmapPath, args);
            let output = "";
            let error = "";
            // Store the process with a unique ID
            const scanId = `${target}`;
            activeNmapProcesses.set(scanId, nmap);
            ...
            nmap.on("close", (code) => {
                // Remove the process from tracking when it completes
                activeNmapProcesses.delete(scanId);
                if (code === 0) {
                    resolve({ success: true, output });
                }
                else {
                    // Check for root privilege errors
                    if (error.includes("requires root privileges") ||
                        error.includes("Permission denied")) {
                        const rootErrorMessage = process.platform === "darwin" || process.platform === "linux"
...
// Handle aborting nmap scans
electron_1.ipcMain.handle("abort-nmap-scan", async (_, scanId) => {
    if (activeNmapProcesses.has(scanId)) {
        const process = activeNmapProcesses.get(scanId);
        // Kill the process
        try {
            if (process && !process.killed) {
                // On Windows, force kill is needed
                if (process.platform === "win32") {
                ...
                activeNmapProcesses.delete(scanId);
                return { success: true, message: "Scan aborted successfully" };
            }
        }
        catch (error) {
            console.error(`Error aborting scan ${scanId}:`, error);
            return {
                success: false,
                error: `Failed to abort scan: ${error.message || "Unknown error"}`,
            };
        }
    }
